...
/** n_decimal (decimal(20,6)): one @Test per JDBC numeric binding. */
@DisplayName("scalar_zoo.n_decimal — JDBC bindings")
class NDecimalBindingTest {

    private static final String COL = "n_decimal";
    private static final Number EXPECT = new BigDecimal("7");

    @Test @DisplayName("setByte")
    void setByte() {
        bindAndCheck(COL, (ps, i) -> ps.setByte(i, (byte) 7),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }

    @Test @DisplayName("setShort")
    void setShort() {
        bindAndCheck(COL, (ps, i) -> ps.setShort(i, (short) 7),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }

    @Test @DisplayName("setInt")
    void setInt() {
        bindAndCheck(COL, (ps, i) -> ps.setInt(i, 7),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }

    @Test @DisplayName("setLong")
    void setLong() {
        bindAndCheck(COL, (ps, i) -> ps.setLong(i, 7L),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }

    @Test @DisplayName("setFloat")
    void setFloat() {
        bindAndCheck(COL, (ps, i) -> ps.setFloat(i, 7.0f),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }

    @Test @DisplayName("setDouble")
    void setDouble() {
        bindAndCheck(COL, (ps, i) -> ps.setDouble(i, 7.0),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }
    ...
    void setString() {
        bindAndCheck(COL, (ps, i) -> ps.setString(i, "7"),
            (rs, c) -> rs.getBigDecimal(c), EXPECT);
    }
}
